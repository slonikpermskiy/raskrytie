											...
											<v-tooltip left max-width="400px" :disabled="premiumstatus">
												<template v-slot:activator="{ on: onTooltip, attrs }">
													...
													<v-chip :color="getStatusColor(event.status)" class="ma-1 me-2" @click.prevent v-bind="attrs" v-on="{...onMenu, ...onTooltip}">
														...
														<div class="ms-2 no-print-margine white--text text-uppercase">{{ getStatusName(event.status) }}</div>

														<v-icon 	
															class="py-2 ps-1 no-print"
															color="white">
				...
				<v-card-text>
					...
					<v-row class="my-0 py-0">
						...
						<v-col
							cols="12"
							md="6"
							v-if="otch_data.otchetstartdate" class="d-flex align-center my-0 py-0">
							
							<v-list-item class="mx-0 px-0">
								<v-list-item-content>
									<v-list-item-subtitle class="indigo--text wrap-text">Дата начала сдачи отчета</v-list-item-subtitle>
									<v-list-item-title class="black--text wrap-text">{{ formatDate(otch_data.otchetstartdate) }}</v-list-item-title>
								</v-list-item-content>
							</v-list-item>			
	...
	export default {
		...
		methods: {
			...
			openOtchData(event) {
				...
				this.otch_data.otchetstartdate = event.otchetdata.otchetstartdate.date;
